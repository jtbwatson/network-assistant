...
def fetch_model_info():
            ...
            if response:
                ...
                if model_info_attr:
                    if hasattr(model_info_attr, "general.basename"):
                        model_info["base"] = getattr(
                            model_info_attr, "general.basename"
                        )
                    elif isinstance(model_info_attr, dict):
                        model_info["base"] = model_info_attr.get(
                            "general.basename", model_info["base"]
                        )

                    try:
                        if hasattr(model_info_attr, "general.parameter_count"):
                            count = int(
                                getattr(model_info_attr,
                ...
                if modelfile and (
                    isinstance(modelfile, str) or hasattr(modelfile, "split")
                ):
                    # Handle both string and string-like objects
                    model_lines = (
                        modelfile.split("\n")
                        if hasattr(modelfile, "split")
                        else str(modelfile).split("\n")
                    )
                ...
                logger.info(
                    f"🟢 Model info retrieved for {model_info['name']}: {model_info['base']}"
                )
                return model_info

        except Exception as e:
            logger.warning(
                f"🟡 Attempt {attempt+1}/{MAX_RETRIES}: Error getting model info: {e}"
            )
    ...
    logger.error(
        f"🔴 Failed to get model information after {MAX_RETRIES} attempts")
    return model_info
...
def generate_response(message, context, history):
    ...
    history_prompt = "\nConversation history:\n" + "\n".join(
        ...
        for msg in history[
            :-1
        ]  # Exclude the latest message which we'll include separately
    )
...
"""
    ...
    for attempt in range(MAX_RETRIES):
        try:
            logger.info(
                f"🟡 Generating response, attempt {attempt+1}/{MAX_RETRIES}")
            ...
            if response:
                ...
                logger.info(
                    f"🟢 Response generated successfully ({len(assistant_response)} chars)"
                )
                return assistant_response, True

        except Exception as e:
            logger.warning(
                f"🟡 Attempt {attempt+1}/{MAX_RETRIES}: Request error: {e}")
...
def check_ollama_connection():
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            logger.info(
                f"🟢 Successfully connected to Ollama at {config.OLLAMA_HOST}")
            ...
            if hasattr(list_response, "models"):
                ...
                if models:
                    ...
                    if model_names:
                        ...
                        if not model_found:
                            logger.warning(
                                f"🟡 Configured model '{config.OLLAMA_MODEL}' not found in available models. Make sure it's downloaded."
                            )
                    else:
                ...
                try:
                    ...
                    if isinstance(list_response, dict) and "models" in list_response:
                        ...
                        logger.info(
                            f"🟢 Available models: {', '.join(filter(None, model_names))}"
                        )
                        ...
                        try:
                            ...
                            logger.debug(
                                f"Response raw string: {list_str[:200]}..."
                            )  # Truncate for log readability
                        except:
                            pass
                        logger.warning(
                            f"🟡 Unexpected response format from ollama.list() - trying to continue"
                        )
                except Exception as e:
            ...
            logger.warning(
                f"🟡 Attempt {attempt+1}/{MAX_RETRIES}: Could not connect to Ollama: {e}"
            )
    ...
    logger.error(
        f"🔴 Could not connect to Ollama at {config.OLLAMA_HOST} after {MAX_RETRIES} attempts"
    )
    return False
