...
class OllamaEmbeddingFunction:
    ...
    def __init__(self, ollama_base_url, model_name, batch_size=10):
        ...
        """
        logger.info(
            f"🟡 Initializing OllamaEmbeddingFunction with URL: {ollama_base_url} and model: {model_name}"
        )
        ...
        try:
            ...
            logger.info(
                f"🟢 Successfully connected to Ollama embedding API at {self.ollama_base_url}"
            )
    ...
    def _test_connection(self):
        ...
        if response and "embedding" in response:
            ...
            logger.info(
                f"🟢 Successfully tested embedding - received embedding of length {len(embedding)}"
            )
        else:
    ...
    def _get_embedding(self, text):
        ...
        try:
            logger.debug(
                f"🟡 Requesting embedding for text of length {len(text)}")
            ...
            if response and "embedding" in response:
                embedding = response["embedding"]
                logger.debug(
                    f"🟢 Got embedding in {elapsed:.2f}s - length: {len(embedding)}"
                )
                return embedding
            else:
    ...
    def __call__(self, input: Union[str, List[str]]) -> List[List[float]]:
        ...
        for i in range(0, len(texts), self.batch_size):
            ...
            if len(texts) > self.batch_size:
                logger.info(
                    f"Processed batch {i//self.batch_size + 1}/{(len(texts)-1)//self.batch_size + 1} in {batch_elapsed:.2f}s"
                )

        total_time = time.time() - start_time
        logger.info(
            f"🟢 Generated {len(all_embeddings)} embeddings in {total_time:.2f}s (avg: {total_time/len(all_embeddings):.2f}s per embedding)"
        )
