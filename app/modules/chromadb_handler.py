...
class ChromaDBStatus:
    ...
    def get_status_dict(self):
        ...
        return {
            ...
            "last_query_time_ms": (
                round(self.last_query_time *
                      1000) if self.last_query_time else None
            ),
    ...
    try:
        ...
        if use_ollama_embeddings:
            try:
                ...
                logger.info(
                    f"🟢 Using Ollama for embeddings: {config.OLLAMA_HOST} with model {config.OLLAMA_EMBEDDING_MODEL}"
                )
                return emb_fn
            except Exception as e:
                logger.error(
                    f"🔴 Error setting up Ollama embedding function: {e}")
...
def init_db():
    ...
    if not config.CHROMA_AVAILABLE:
        logger.warning(
            "🔴 ChromaDB not available. Vector search will be disabled.")
        return None, None
    ...
    try:
        ...
        for retry_count in range(MAX_RETRIES):
            try:
                logger.info(
                    f"🟡 Initializing ChromaDB (attempt {retry_count + 1}/{MAX_RETRIES})"
                )

                # Create persistent client
                client = chromadb.PersistentClient(
                    path=config.DB_DIR, settings=chroma_settings
                )
                ...
                try:
                    collection = client.get_collection(
                        name=db_status.collection_name, embedding_function=emb_fn
                    )
                    logger.info(
                        f"🟢 Found existing collection: {db_status.collection_name}"
                    )
                except ValueError:
                    ...
                    logger.info(
                        f"🟢 Created new collection: {db_status.collection_name}"
                    )
                ...
                logger.info(
                    f"🟢 ChromaDB initialized successfully in {db_status.initialization_time:.2f}s with {db_status.document_count} documents"
                )
        ...
        logger.error(
            f"🔴 Error initializing ChromaDB after {MAX_RETRIES} attempts: {db_status.last_error}"
        )
        return None, None
